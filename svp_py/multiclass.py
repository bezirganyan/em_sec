...
retrieved from: https://github.com/tfmortie/setvaluedprediction
...
"""
import torch
import time
...
# Instead of loading the C++ extension, load our Python version:
from svp_py.svp_py import SVP
from svp_py._utils import _C  # _C may be None now (see below)
from svp_py.utils import LabelTransformer, FLabelTransformer, HLabelTransformer, PriorityQueue
from sklearn.base import BaseEstimator, ClassifierMixin, clone
from sklearn.utils import check_X_y, check_array
from sklearn.utils._user_interface import _message_with_time
...
class SVPNet(torch.nn.Module):
    ...
    def calibrate(self, X, y, params):
        ...
        if params["svptype"] == "lac":
            ...
            o.extend(list(self.SVP.calibrate_raps(X, y, params["error"], params["rand"], params["lambda"], params["k"],
                                                  params["c"])))
            ...
            y = torch.Tensor(self.transformer.transform(y, False)).long()
            o.extend(list(
                self.SVP.calibrate_csvphf(X, y, params["error"], params["rand"], params["lambda"], params["k"],
                                          params["c"])))
            ...
            y = torch.Tensor(self.transformer.transform(y, False)).long()
            o.extend(
                list(self.SVP.calibrate_crsvphf(X, y, params["error"], params["rand"], params["lambda"], params["k"])))
        else:
            warnings.warn(
