...
def edl_hyperloss(func, y, alpha, hyperset_soft_size, epoch_num, num_classes, annealing_step, device, useKL=True, lmda=1):
    y = y.to(device)
    alpha = alpha.to(device)
    S = torch.sum(alpha, dim=1, keepdim=True)
    # normalizer of ones for each class, but (1 / hyperset_soft_size) for the last class
    hyperset_normalizer = torch.ones_like(y)
    # hyperset_normalizer[:, -1] = hyperset_soft_size * lmda
...
def get_evidential_hyperloss(evidence, multilabel_probs, target, epoch_num, num_classes, annealing_step, device):
    ...
    alpha_a = evidence + 1
    hyperset_soft_size = torch.sigmoid(10 * (multilabel_probs - 0.5)).sum(dim=1)
